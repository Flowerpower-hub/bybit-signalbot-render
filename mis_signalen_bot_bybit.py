...
laatste_signalen = {}        # laatste hoofd-signalen (15m)
bevestigde_signalen = {}     # of 15m al bevestigd is met 5m
...
def fetch_data(exchange, symbol, timeframe):
    try:
        ...
        send_telegram(f"⚠️ STORING bij ophalen {symbol} ({timeframe})\n❌ {e}")
        return []
        ...
        for symbol in symbols:
            ...
            if analyse_5m:
                ...
                if sleutel in bevestigde_signalen:
                    ...
                    if (nu - tijd_15m).total_seconds() < 1800:  # binnen 30 min
                        if check_entry(analyse_5m, richting):
                            send_telegram(f"""🔁 *{symbol}*\nTimeframe: 5m\nRichting: {richting.upper()}\nBevestiging van eerder 15m-signaal""")
                            del bevestigde_signalen[sleutel]  # één keer bevestigen
